...
> **레트로 게임 에뮬레이터의 새로운 경험**

웹 브라우저에서 Game Boy 게임을 즐길 수 있는 완전한 에뮬레이터입니다. 순수 JavaScript로 구현된 CPU, GPU, APU, 메모리 시스템을 통해 원본 하드웨어를 재현하면서도 현대적인 웹 기술로 접근성을 높였습니다.
...
![Game Boy Memory Map](assets/memory-map.png)
...
이렇게 메모리 맵을 기반으로 하드웨어를 구현하기 시작하고 **메모리 맵이 하드웨어의 인터페이스**라는 것을 체감할 수 있었습니다. 모든 하드웨어 컴포넌트(CPU, PPU, APU, 타이머 등)가 메모리를 통해 통신하기 때문에, 메모리 맵을 정확히 구현하는 것이 전체 시스템의 정확성을 좌우했습니다.

### 실시간 에뮬레이션 성능 최적화

게임 에뮬레이터에서 가장 중요한 것은 정확성과 성능의 균형이었습니다. Game Boy의 4.19MHz CPU를 웹 브라우저에서 실시간으로 에뮬레이션하면서도 60fps의 부드러운 화면을 보여줘야 하는 것은 큰 도전이었습니다.

그럼 Retro Box에서는 어떻게 실시간 에뮬레이션 성능을 최적화했을까요? 이상적으로는, 모든 CPU 사이클을 정확히 계산하고, PPU가 매 프레임마다 144라인을 스캔하며, APU가 4채널 사운드를 실시간으로 처리하면 되겠죠.
...
사실 저도 개발 초기에는 이 점을 체감했는데요, Game Boy의 CPU는 4.19MHz로 동작하며 매 사이클마다 정확한 타이밍이 필요합니다.
...
이 사실에 슬퍼하면서도, 저는 동시에 답변의 마지막 문장에 주목했습니다: 잘 디자인된 에뮬레이터는 획기적인 차이를 만들어낼 수 있습니다. 정확성과 성능의 균형을 맞추는 것이 핵심이었죠.

물론 Retro Box가 상용 에뮬레이터 수준의 최적화를 달성할 순 없겠지만, 어떻게 하면 정확성을 유지하면서도 성능을 최대한 높일 수 있을지 고민했습니다.

### 프레임 기반 처리로 성능 최적화

CPU 사이클 정확성을 유지하면서도 성능을 높인다는 것은 곧, 프레임 단위로 처리하되 중요한 타이밍은 정확히 맞추는 것입니다. Game Boy는 60fps로 동작하며, 한 프레임은 70224 CPU 사이클로 구성됩니다.

하지만 역설적이게도, 모든 70224 사이클을 정확히 계산하면 브라우저가 블로킹되는 듯 했습니다. 매 사이클마다 CPU, GPU, APU, 타이머를 업데이트해야 하기 때문이죠.

그 순간, 저는 Game Boy의 하드웨어 특성을 활용한 해결책을 떠올렸습니다. Game Boy는 V-Blank 인터럽트를 통해 프레임 동기화를 하며, 대부분의 게임이 이 타이밍에 중요한 업데이트를 수행합니다.
...
그런데 뭔가 이상하지 않나요? 각 스캔라인이 정확히 456 CPU 사이클(114 T-Cycles)로 구성되어야 하는데, 실제로는 스프라이트 개수와 윈도우 렌더링에 따라 Drawing 모드의 시간이 가변적이었습니다.

이는 바로 PPU의 복잡한 타이밍 시스템 때문이었습니다. 그래서 저는 스캔라인 기반 렌더링을 도입하여 정확한 타이밍을 구현했습니다.
