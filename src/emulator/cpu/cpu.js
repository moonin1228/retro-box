...
export function createGameBoyCPU() {
  return {
    registers: {
      A: 0x01,
      F: 0xb0,
...
export function read8Reg(cpu, reg) {
  return cpu.registers[reg] & 0xff;
}

export function write8Reg(cpu, reg, value) {
  if (reg === "F") {
...
export function read16Reg(cpu, regPair) {
  switch (regPair) {
    case "BC":
...
export function write16Reg(cpu, regPair, value) {
  value &= 0xffff;
  switch (regPair) {
    case "BC":
...
export function readMemory(cpu, address) {
  return read8(address);
}

export function writeMemory(cpu, address, value) {
  write8(address, value);
}

export function readMemory16(cpu, address) {
  return read16(address);
}

export function writeMemory16(cpu, address, value) {
  write16(address, value);
}

export function getFlag(cpu, flag) {
  return (cpu.registers.F & flag) !== 0;
}

export function setFlag(cpu, flag, value) {
  if (value) {
...
export function setFlags(cpu, { z = null, n = null, h = null, c = null }) {
...
export function halt(cpu) {
  cpu.halted = true;
}

export function enableInterrupts(cpu) {
  cpu.interruptsEnabled = true;
}

export function disableInterrupts(cpu) {
  cpu.interruptsEnabled = false;
}

export function addCycles(cpu, cycles) {
  cpu.cycles += cycles;
}
